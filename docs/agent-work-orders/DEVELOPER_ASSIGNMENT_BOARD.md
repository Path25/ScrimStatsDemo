...
|---|---|---|---|
| [WO-2026-040](proposed/WO-2026-040-discord-production-readiness-controlled-release.md) | Core Features Developer / QA and Release Auditor | Independent QA review supports the source-only timing correction: `waitUntil` scheduling returns the authorised response before exact replay/evidence completion; focused contracts, lint, and TypeScript pass. Hosted v12 remains unchanged and failed at 3,184 ms. | Core must provide exact deployment scope. Theo must separately approve deployment, then a fresh ≤15-minute arm and one non-overlapping private provider command. Do not activate workers, send delivery, touch customers, change production configuration, or begin later phases. |
