...
const healthPresentation: Record<DiscordDeliveryHealthState, { label: string; description: string; tone: string }> = {
  ...
  configured: { label: "Configured - test only", description: "Schedule prompts are configured, but customer delivery is not released.", tone: "border-[#8994ff]/35 bg-[#8994ff]/10 text-[#aeb5ff]" },
      ...
      <div className="border-b border-[var(--workspace-rule)] p-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-2 max-w-2xl border-l-2 border-[var(--workspace-awaiting)] pl-3 text-xs leading-5 text-[var(--workspace-subtle)]">Test-only configuration. Discord is not available to customer workspaces until hosted QA, a consented pilot, and Theo's separate release approval are complete.</p>
          </div>
        </div>
      </div>
