...
|---|---|---|---|
| [WO-2026-036](proposed/WO-2026-036-elite-opponent-preparation-playbooks.md) | Core Features Developer | Phase 2B is locally implemented: staff-only Scouting workflow and bounded Draft/Action/Scrim breadcrumbs; exact isolated migration/behavior validation, zero-warning DB lint, 243/243 repository tests, lint, TypeScript, build, and bundle budget pass. Hosted migration, type regeneration, deployment, authenticated matrix, Advisor review, and QA audit remain open and separately approval-gated. | Dedicated `opponent_preparation` stream. Preserve Pro Scouting/Draft; no external sources/providers/links/embeds/scraping/VODs and no broad shared-workflow refactor. |
