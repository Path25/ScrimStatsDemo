...
|---|---|---|---|
| [WO-2026-040](proposed/WO-2026-040-discord-production-readiness-controlled-release.md) | Core Features Developer / QA and Release Auditor | Commit `dd354f8` is pushed and deployed as active `discord-interactions` v13, SHA `0346de39...6fdb`; retrieved source exactly matches the timing candidate, unsigned POST fails closed with 401, replay controls/workers are inactive, and fixture counts are unchanged. | QA must independently verify the hosted revision and inert state. A fresh ≤15-minute arm and one non-overlapping private provider command require separate Theo approval; do not activate workers, send delivery, touch customers, change configuration, or begin later phases. |
