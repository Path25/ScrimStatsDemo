...
export const pair = (pairingCode: string, deviceLabel: string) => request('/collector-pair', { method: 'POST', body: JSON.stringify({ pairing_code: pairingCode, device_label: deviceLabel, app_version: '0.2.0-beta' }) }) as Promise<{ device_id: string; credential: string; tenant_id: string; scrims: ScheduledScrim[]; roster: RosterIdentity[] }>;
const auth = (credential: Credential) => ({ Authorization: `Bearer ${credential.credential}`, 'X-Collector-Device-ID': credential.deviceId });
export const heartbeat = (credential: Credential) => request('/collector-ingest', { method: 'POST', headers: auth(credential), body: JSON.stringify({ action: 'heartbeat', app_version: '0.1.0-beta' }) });
export const startSession = (credential: Credential, scrimId: string, clientSessionId: string, localGameId: string) => request('/collector-ingest', { method: 'POST', headers: auth(credential), body: JSON.stringify({ action: 'start', scrim_id: scrimId, client_session_id: clientSessionId, local_game_id: localGameId, schema_version: 1 }) });
