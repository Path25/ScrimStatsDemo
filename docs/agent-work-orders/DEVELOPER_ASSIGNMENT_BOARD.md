...
| [WO-2026-037](proposed/WO-2026-037-membership-first-workspace-entry-and-creation.md) | QA and Release Auditor | Fail-closed current-user membership state, stale-request protection, retryable unavailable routes, and owner-eligible selector creation entry; focused 16/16, repository 233/233, zero-warning lint, TypeScript, production build, and bundle budget. No SQL, RPC, hosted mutation, deployment, or authenticated-browser claim. | Review source/local evidence first. A staging deployment and controlled one/multi/no-membership plus member/admin/viewer-only role matrix require separate approval and suitable sessions. |
