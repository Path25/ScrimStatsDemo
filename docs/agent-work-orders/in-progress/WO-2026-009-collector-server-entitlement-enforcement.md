...
- **Prepared release commit:** `937fcdb` (`Fix Free Collector state in Integrations`) contains only the four WO-009 files above and is recorded as the tracked `origin/codex/Staging` revision. This triggers the existing staging-branch deployment path.
- **Deployment status:** Git branch delivery is recorded; Vercel Ready status and the authenticated Free-browser recheck are still unverified. A later attempt to push the documentation-only evidence commit failed with HTTP `401 Unauthorized` because this environment has no usable GitHub credential and cannot open an interactive credential prompt.
- **Theo handoff:** Confirm the Vercel deployment for `937fcdb` reaches Ready, then ask QA to perform the Free `/integrations` recheck. Optionally authenticate Git for `https://github.com/Path25/ScrimStatsDemo.git` in this workspace, or push the one remaining documentation-only commit from your own authenticated environment.

## QA staging re-audit - 2026-07-29 (post `937fcdb`)

- **Verdict:** HOLD. The corrected Free capture-profile state is staged, but this work order is not release-ready because its server-side and lifecycle acceptance criteria still lack authenticated behavioural evidence.
- **Pass â€” staged Free browser:** The dedicated Free owner in `TestWorkspace` loaded `/collector` and received only the Pro gate (`Unlock collector capture` and `Compare plans`), with no browser-console errors. On `/integrations`, `Game Capture + Manual` now correctly shows `Pro feature`, the Pro explanation, and no `Active` state; `GRID + Manual` remains the inactive manual-capable option. This verifies the developer correction is deployed to the staging hostname.
- **Important â€” remaining paid-copy contradiction:** The same Free `/integrations` screen still renders an `Available` badge beside the `Game Capture` section immediately above its `Pro feature` lock. The underlying component renders that badge from `modules.collector.state` and ignores `modules.collector.enabled`; it is therefore a customer-visible availability claim that conflicts with the lock. Change the badge/copy for a non-entitled workspace to an honest Pro state (or omit it), then repeat this Free check.
- **Verified hosted configuration:** Supabase project `tvcgjehreaayfazlhvps` contains migration `20260729081717` and `tenants.subscription_past_due_started_at`. Active deployed revisions inspected by QA are `collector-pairing` v6, `collector-status` v5, and `collector-ingest` v12. Their shared helper evaluates paid tier, active/trial status, paid-period end, and the seven-day first-past-due timestamp; Free denial uses the customer-safe `collector_plan_required` 403 response.
- **Blocking â€” direct and lifecycle matrix remains unverified:** No authenticated call has yet exercised Free denial for pairing, pairing-code redemption, status/configuration, or device ingest; no Pro manager has exercised the allowed pairing/status path; and no paired test device has been tested across cancellation/downgrade or past-due day-seven expiry. Deployed source and a browser lock do not prove request authentication, tenant isolation, Stripe event ordering, or existing-device revocation.
- **Exact remaining checks Theo must approve or complete:** (1) sign into the dedicated Pro manager workspace on staging and tell QA `Pro ready`; (2) approve a temporary test pairing artefact and its immediate revocation/deletion so QA can capture the direct Free/Pro pairing, redemption, status/configuration, and device-ingest responses; (3) approve an isolated Stripe test-mode lifecycle mechanism (or explicitly approve the connected production-project test scope) to rehearse cancellation/downgrade paid-period-end retention and past-due warning/expiry; (4) after the badge fix, request a Free `/integrations` recheck. No customer workspace or customer subscription state will be used without that explicit approval.

## Developer badge and lifecycle-copy correction - 2026-07-29

- **Outcome:** Corrected the QA-reported Free `Available` badge and the related browser lifecycle drift. Game Capture UI now consumes the existing lifecycle-aware `tenant.collectorEntitled` result rather than the subscription tier alone.
- **Changed files:** `src/components/workspace/ModuleStateBadge.tsx`, `src/pages/Integrations.tsx`, `src/pages/Settings.tsx`, and `scripts/collector-entitlement-contract.test.mjs`.
- **Behaviour:** `ModuleStateBadge` accepts an optional enabled state and alternate unavailable label. Integrations supplies both the lifecycle entitlement and persisted Collector-module state; a Free or expired past-due workspace now receives `Pro feature`, not `Available`. Settings uses the same lifecycle entitlement for its Game Capture link and copy. Other module badges retain their existing behaviour.
- **Validation:** Collector contract 3/3 passed; full automated suite 175/175 passed; TypeScript passed; ESLint passed with zero warnings; production Vite build and bundle budget passed. The build emitted only the existing stale Browserslist-data notice.
- **Unverified:** No authenticated desktop/tablet/mobile browser run exercises this changed paid state locally, and no staging deployment or hosted recheck has occurred.
- **Exact QA handoff:** After Theo separately approves staging deployment, use the dedicated Free `TestWorkspace` at `/integrations` and `/settings` to confirm Game Capture never renders `Available` or `Active`; it must show the Pro state and must not link Free to `/collector`. Repeat the lifecycle UI check with a Pro/Elite workspace and, when an approved isolated past-due test exists, after the seven-day expiry. Retain the existing direct Function, device, tenant-isolation, and Stripe lifecycle matrix requirements.

## QA staging re-audit - 2026-07-29 (badge and lifecycle-copy correction)

- **Verdict:** HOLD pending the existing authenticated direct-path and lifecycle matrix. The staging UI correction passes; this is not sufficient evidence for the server entitlement release.
- **Pass â€” dedicated Free owner on staging:** `TestWorkspace` loaded `/integrations` with `Game Capture + Manual` labelled `Pro feature` and the Game Capture module badge also labelled `Pro feature` (not `Active` or `Available`). It showed the Pro preview and no Game Capture open link. `/settings` showed the Free billing contract, the Game Capture Pro explanation, and only `View integration capabilities` linking to `/integrations`, not `/collector`.
- **Pass â€” loading/error state:** Free Collector connection status settled from its transient check to `Unavailable` / `Collector unavailable`; no browser-console errors were recorded. This rules out a persistent loading regression in the checked Free browser journey.
- **Verified source and local test:** `tenant.collectorEntitled` is now used for the Integrations and Settings capture state, and the module badge accepts an enabled state with an unavailable label. `node --test scripts/collector-entitlement-contract.test.mjs` passed 3/3.
- **Verified hosted configuration:** The lifecycle migration remains applied and `subscription_past_due_started_at` exists. Active Collector revisions remain pairing v6, pair v4, status v5, and ingest v12; these remain source/configuration evidence, not behavioural proof.
- **Blocking / unverified:** Free direct pairing, pairing-code redemption, status/configuration, and device-ingest 403 responses; Pro direct allowance; tenant isolation; a paired-device denial after cancellation/downgrade expiry; and the seven-day past-due warning/expiry have still not been exercised. No test artefact or subscription state was created by QA.
- **Exact remaining Theo checks or approvals:** (1) sign into the dedicated Pro manager account on staging and say `Pro ready`; (2) approve creation and immediate revocation/deletion of a temporary test pairing artefact for the direct pairing/device checks; (3) approve an isolated Stripe lifecycle rehearsal, or explicitly approve this test-only state change against the connected production project. Until those checks are recorded, do not mark this paid-entitlement work order READY or deploy it to production.

## QA authenticated Pro browser recheck - 2026-07-29

- **Verdict:** HOLD. The supplied dedicated Pro owner session passes the paid browser journey, but the release acceptance criteria still require direct server-path, tenant-isolation, and lifecycle evidence.
- **Pass — paid UI, billing, and connection state:** On `staging.scrimstats.gg`, the authenticated Pro workspace loaded `/collector` and showed `Collector ready` with the supported Windows-app handoff. `/integrations` showed Game Capture as `Available` with `Open Game Capture`; `/settings` identified Pro as the current active plan, included Game Capture in the Pro contract, and linked to the Collector journey. Its connection state settled to `Connected and ready for a game`. No browser console errors were recorded across these checks.
- **Pass — Free/Pro paid-state coherence:** This completes the browser-state comparison against the earlier dedicated Free recheck: Free is shown a Pro gate and has no Collector link; Pro is shown an available, linked Collector journey. No customer data, pairing artefact, device credential, or billing/subscription state was created or changed by QA.
- **Blocking / unverified:** The browser session does not directly invoke or prove `collector-pairing`, pairing-code redemption, device-token status/configuration, or `collector-ingest`; it therefore cannot establish request authentication, tenant isolation, customer-safe Free denial, authenticated Pro allowance, or revocation of an already-paired device. Cancellation/downgrade paid-period-end retention and the manager-only past-due warning/seven-day expiry also remain unexercised.
- **Exact remaining Theo approvals:** (1) approve a temporary, non-customer pairing artefact and its immediate revocation/deletion so QA can record Free and Pro direct pairing, redemption, status/configuration, and ingest outcomes; (2) approve an isolated Stripe lifecycle rehearsal, or explicitly approve the narrow test-only subscription-state changes against the connected production Supabase project. The Pro sign-in prerequisite is now complete. Do not approve production release until the resulting evidence is attached to this work order.

## Theo risk decision - 2026-07-29

- Theo accepts deferral of the isolated Stripe lifecycle rehearsal. Cancellation/downgrade paid-period-end retention and past-due warning/seven-day expiry remain **unverified**, not passed; this accepted risk must accompany any PM release decision.
- **Single remaining QA approval:** Theo must approve a temporary, non-customer pairing artefact and its immediate revocation/deletion. QA will use only the supplied Free and Pro test workspaces to verify the direct pairing, pairing-code redemption, status/configuration, and ingest enforcement paths, then revoke the artefact and record only response/status evidence.

## QA approved pairing-artifact run - 2026-07-29

- **Scope and safety:** Theo approved this run. QA used only the dedicated Free and Pro test workspaces, created no game/capture data, and did not inspect or retain roster, scrim, browser-session, or device-credential contents. The temporary artefacts were revoked after the checks.
- **Pass — Free server enforcement:** A deliberately seeded temporary Free pairing code was submitted to deployed `collector-pair`. It returned HTTP `403` with `collector_plan_required`; no Free device was created.
- **Pass — Pro supported device path:** A temporary Pro code was redeemed by deployed `collector-pair` with HTTP `200`. Using the returned credential internally (never recorded in QA evidence), deployed `collector-ingest` returned HTTP `200` for both the non-capturing `heartbeat` and `configuration` actions. The configuration response had the expected `scrims` and `roster` shape; their contents were not read or recorded.
- **Pass — cleanup:** QA revoked two temporary Pro devices and all three temporary pairing codes, including the first retry artefact created after an initial harness-header error. Final database confirmation: `0` active QA devices, `2` revoked QA devices, `0` active QA codes, and `3` revoked QA codes.
