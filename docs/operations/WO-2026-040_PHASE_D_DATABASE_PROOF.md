...
- Candidate migration: `20260806142907_wo040_discord_worker_reactivation.sql`. It preserves the existing disable routine, explicitly calls `cron.alter_job(..., active := true)` for both returned job IDs, re-reads each named row's actual state, and fails transactionally unless both are active.
