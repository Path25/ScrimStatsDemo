...
`discord-interactions` v12 is deployed with intentional `verify_jwt=false`: Discord Ed25519 signature verification is mandatory before parsing or lookup, and its only intended command is the server-gated `/scrim` practice-block workflow. Its private WO-040 exact-replay control is inert unless a database operator separately arms one tenant-bound run for at most fifteen minutes; browser and service roles cannot arm it.
...
WO-2026-040 candidate `e268298` is deployed to the shared Supabase project for staging QA: migration history records `20260804134309_discord_production_delivery_controls` and `discord-dispatch` is version 15. Phase A exact-replay source is also hosted as migration `20260805134134_discord_qa_replay_controls` and `discord-interactions` v12, SHA `b31bc919dea39da4a40080b574c5ec5179082e6f236b0dcb09df7f0c72c8cfae`. The replay table is empty and unarmed. Both Discord cron jobs remain inactive, so this is not a provider invocation or customer activation. Arming a private replay run, submitting a Discord interaction, re-enabling workers, smoke/pilot work, and release each retain their recorded separate approval gates.
