...
| [WO-2026-040](proposed/WO-2026-040-discord-production-readiness-controlled-release.md) | QA and Release Auditor | Exact-replay source `0db1a9b` is active as `discord-interactions` v8 with unchanged reviewed SHA; only the private non-customer guild allowlist is configured. Baseline: 5 scrims, 2 receipts, 7 Discord events, 1 pending event, 6 attempts, 0 active workers. | Submit one permitted, non-overlapping private `/scrim` with exact notes `WO-040 EXACT REPLAY`; prove one internal signed replay, no duplicate receipt/scrim/outbox row, no delivery attempt, and then return to Core for immediate fixture removal. Release remains HOLD. |
|---|---|---|---|
