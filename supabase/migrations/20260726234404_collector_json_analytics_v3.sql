-- Expose Collector v3 game context to the tenant-authorized analytics dataset.
-- This remains additive to team-analytics-v2 so existing clients keep working.
...
as $$
  with scoped_games as (
    select
      ...
      case
        when game.external_game_data -> 'game_context' ->> 'map_number' ~ '^[0-9]+$'
          then (game.external_game_data -> 'game_context' ->> 'map_number')::integer
        else null
      end as map_number
  ...
  select jsonb_build_object(
    ...
    'filter_options', jsonb_build_object(
      ...
      'game_modes', coalesce((
        select jsonb_agg(option_row.game_mode order by option_row.game_mode)
        from (select distinct game_mode from scoped_games where game_mode is not null) option_row
      ), '[]'::jsonb),
      'maps', coalesce((
        select jsonb_agg(to_jsonb(option_row) order by option_row.map_name)
        from (
...
comment on function public.get_team_analytics_dataset(uuid, date, date) is
  'Returns normalized factual analytics rows, including Collector v3 game mode and map context, without exposing raw provider payloads.';
