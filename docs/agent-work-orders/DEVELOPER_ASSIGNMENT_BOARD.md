...
|---|---|---|---|
| [WO-2026-040](proposed/WO-2026-040-discord-production-readiness-controlled-release.md) | Core Features Developer / QA and Release Auditor | Exact-replay source `0db1a9b` is deployed as `discord-interactions` v7 and workers remain paused, but the fixture is inert until its private QA-guild allowlist is configured in an authorised Supabase Dashboard session. | Theo signs in to the project Dashboard, then Core configures only `DISCORD_QA_REPLAY_GUILD_ID`, confirms paused workers, and hands the exact-marker scenario to QA. Do not activate workers, configure production delivery, use customer data, or claim release readiness. |
